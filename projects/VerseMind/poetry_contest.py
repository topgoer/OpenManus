...
from poetry_llm import PoetryLLM, get_available_models
...
class AIPoetryContest:
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            for poet in self.poets:
                try:
                    # 格式化提示词
                    prompt = self.creation_prompt.replace("{theme}", self.reference_content)
            ...
            for poet in self.poets:
                for poem_info in self.anonymous_poems:
                    try:
                        ...
                        if is_human_poem:
                            logger.info(f"{poet.model_name} 正在评价人类诗歌，ID: {poem_info['id']}")
                        ...
                        if evaluation:
                            ...
                            logger.info(f"{poet.model_name} 完成了对诗歌 {poem_info['id']} 的评价")
                        else:
                ...
                if poems_count > 0:
                    ...
                    if similar_poems:
                        ...
                        for i, (poem, similarity) in enumerate(similar_poems):
                            logger.info(f"  相似度 {similarity:.2f}: {poem['content'][:50]}...")
            else:
            ...
            try:
                ...
                assert len(saved_poems) == len(self.anonymous_poems), "保存的匿名诗歌数量与内存中的不一致"
                if human_poem_added:
                    assert any(poem["model"] == "human" for poem in saved_poems), "保存的匿名诗歌中不包含人类诗歌"
            except Exception as e:
            ...
            try:
                ...
                for poet in self.poets:
                    if poet.evaluations:
                        evaluation_file = self.results_dir / f"{poet.name}_evaluations.json"
    ...
    def add_human_poem(self, poem_content: str) -> bool:
        ...
        try:
            ...
            # 断言人类诗歌已成功添加到匿名诗歌列表
            assert any(poem["model"] == "human" for poem in self.anonymous_poems), "人类诗歌未能成功添加到匿名诗歌列表"
    ...
    def _save_results(self) -> bool:
        ...
        try:
            # 保存匿名诗歌
            with open(self.results_dir / "anonymous_poems.json", "w", encoding="utf-8") as f:
                json.dump(self.anonymous_poems, f, ensure_ascii=False, indent=2)
            ...
            with open(self.results_dir / "evaluation_results.json", "w", encoding="utf-8") as f:
    ...
    def search_similar_poems(self, query: str, k: int = 5) -> List[Tuple[Dict[str, Any], float]]:
        """搜索相似诗歌
