...
class PoetryLLM(LLM):
    ...
    async def _send_request(self, data: Dict[str, Any], is_stream: bool = False) -> str:
        ...
        try:
            ...
            # 记录请求信息
            logger.info(f"[PoetryLLM] HTTP Request: POST {api_endpoint} [{request_stage}阶段 第{request_count}次请求]")
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    logger.debug(f"[PoetryLLM] 响应内容: {response_text[:200]}...")  # 只记录前200个字符
                    ...
                    if data.get("stream", False):
                        ...
                        try:
                            ...
                            logger.error(f"[PoetryLLM] JSON解析错误: {str(e)}, 响应内容: {response_text[:200]}...")
                            raise
                    ...
                    logger.error(f"[PoetryLLM] HTTP状态错误: {str(e)}, 状态码: {e.response.status_code}")
                    raise
    ...
    async def create_poem(self, theme: str, prompt: Optional[str] = None) -> Optional[str]:
        """创建诗歌
        ...
        try:
            ...
            try:
                ...
                response = await self.ask([{"role": "user", "content": prompt}], stream=False)
            ...
            # 创作阶段结束，记录请求数
            logger.info(f"[PoetryLLM] 创作阶段完成，共发送 {PoetryLLM._creation_request_count} 个请求")
    ...
    async def evaluate_poem(self, poem: str, theme: str, is_human_poem: bool = False, prompt: Optional[str] = None) -> Optional[Dict]:
        """评估诗歌
        ...
        try:
            ...
            if prompt:
                ...
                prompt = prompt_template.replace("{theme}", reference_content).replace("{poem}", cleaned_poem)
            ...
            try:
                ...
                # 评论阶段结束，记录请求数
                logger.info(f"[PoetryLLM] 评论阶段完成，共发送 {PoetryLLM._evaluation_request_count} 个请求")
    ...
    async def ask(
        ...
        try:
            ...
            if self.api_type == "ollama":
                ...
                # 记录请求参数
                logger.debug(f"[PoetryLLM] 请求参数: {json.dumps(params, ensure_ascii=False)}")
                    ...
                    for line in lines:
                        if line.strip():
                            try:
                                ...
                                logger.error(f"[PoetryLLM] JSON解析错误: {str(e)}, 行: {line}")
                                continue
    ...
    def reset_counters(cls) -> None:
        """重置所有计数器，在整个比赛结束后调用"""
        logger.info(f"[PoetryLLM] 当前请求计数 - 创作阶段: {cls._creation_request_count}, 评论阶段: {cls._evaluation_request_count}")
    ...
    def configure_logging(cls) -> None:
        ...
        class TokenEstimateFilter(logging.Filter):
            def filter(self, record):
                if "Estimated completion tokens for streaming response" in record.getMessage():
                    # 将消息级别改为 DEBUG
